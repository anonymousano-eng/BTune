...
class Application:
    ...
    def set_parameters(self, parameters: Mapping[str, Any]):
        ...
        #self.max_item_size = parameters["max_item_size"]
        ...
        #self.idle_timeout = parameters["idle_timeout"]
        self.listen_backlog = parameters["listen_backlog"]

    def run(self):
        #subprocess.run(["pkill", "memcached"], stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
...
def calculate_reward(metrics: Mapping[str, Any]) -> float:
    ...
    return (throughput / (latency + 1)) / 100  # 缩放防止爆炸
...
def main() -> None:
    ...
    parameters_to_tune = [
        ...
        Integer("port_TCP", val=11211, min=1, max=65535),  # TCP port
        Integer("udp_port", val=0, min=0, max=65535),  # UDP port
        Integer("memory", val=64, min=1, max=32768),  # Max memory in MB
        #Integer("max_item_size", val=1048576, min=1024, max=134217728),  # Max item size (1MB-128MB)
        Integer("max_reqs_per_event", val=20, min=10, max=100),  # Max requests per event
        Real("slab_growth_factor", val=1.25, min=1.0, max=2.0),  # Slab growth factor
        Categorical("unix_mask", val="0700", categories=("0700", "0755", "0777")),  # Unix socket mask
        Categorical("lock_memory", val="off", categories=("on", "off")),  # Lock memory
        ...
        Integer("verbose", val=1, min=0, max=3),  # Verbosity level
    ]
