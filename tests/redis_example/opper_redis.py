...
import csv, os, signal, subprocess, time, logging, redis, psutil
from oppertune.algorithms.hybrid_solver import HybridSolver
from oppertune.core.values import Integer, Categorical, Real
...
ParameterTypes = Union[Integer, Categorical, Real]
...
def main():
    ...
    params = [
        Integer("hz", 10, 10, 50),
        Integer("tcp-backlog", 511, 128, 2048),
        Categorical("appendonly", "yes", ["no", "yes"]),
        Categorical("maxmemory-policy", "noeviction",
                    ["noeviction", "allkeys-lru", "allkeys-random", "volatile-lru"]),
        Integer("tcp-keepalive", 300, 0, 3600),
        Integer("timeout", 0, 0, 300),
    ]
    ...
    solver = HybridSolver(
        ...
        numerical_algorithm_args={"eta": 0.001, "delta": 0.1, "feedback": 2},
    )

    with open("hybrid_result.csv", "w", newline="") as fp:
